...
class WSListener:
	...
	def payloads(self, interface_filter=None, target_os='both'):
		# The generated WebSocket payload launches /bin/sh, so it is only
		# applicable to Unix-like targets. Keep the Listener payload contract
		# aligned with TCPListener.payloads after the upstream OS selector.
		if target_os not in ('linux', 'both'):
			return ""
